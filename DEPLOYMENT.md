...
- `ADMIN_TOKEN` — **required in production** (≥16 chars). The server refuses to start without it because `/api/admin/*` exposes customer PII. Verified. Open the dashboard with `/admin?token=YOUR_ADMIN_TOKEN`; the token is stored in `sessionStorage` and stripped from the URL (the page also prompts for it on a `401`).
